...
function getMaturityDate(fcn) {
  ...
  if (fcn.maturity_time) {
    const d = toDate(fcn.maturity_time);
    if (d) return d;
  }
...
//
// 升級：
// 1. 優先吃 fcn.early_exit_record
// 2. 若沒有，再 fallback 到 localStorage
// 3. 回傳 early_exit_record，供 UI / JSON 預覽使用
// ------------------------------
...
function calcEarlyExit(fcn, marketRuntime = {}, state = {}) {
  ...
  // 先抓 fcn_pool 裡的 record
  const fcnRecord = (fcn && typeof fcn.early_exit_record === "object" && fcn.early_exit_record)
    ? JSON.parse(JSON.stringify(fcn.early_exit_record))
    : {};

  // 再抓 localStorage 舊版 bucket（相容舊資料）
  if (!state[fcnId]) state[fcnId] = {};
  const legacyBucket = state[fcnId];

  const mergedRecord = {};

  const stocks = basket.map(symbol => {
    const fromFCN = fcnRecord[symbol] || {};
    const fromLegacy = legacyBucket[symbol] || {};
    ...
    const prevDate =
      fromFCN.first_hit_time ||
      fromLegacy.remark_date ||
      "";
    ...
    const hit = prevHit || canRemarkToday;
    const firstHitTime = prevDate || (canRemarkToday ? formatDate(now) : "");

    mergedRecord[symbol] = {
      hit,
      first_hit_time: firstHitTime
    };

    // 同步回舊 localStorage 結構，維持相容
    legacyBucket[symbol] = {
      met: hit,
      remark_date: firstHitTime
    };
...
//
// 升級：
// 1. 優先吃 fcn.has_ki_breach / first_ki_breach_time
// 2. 若沒有，再 fallback 到 localStorage
// ------------------------------
function updateKnockInState(fcn, stocks, knockInState) {
  ...
  const prevDate =
    fcn.first_ki_breach_time ||
    bucket.knock_in_date ||
    "";

  const knockedToday = (stocks || []).some(s => toNumber(s.price_now) < toNumber(s.ki_price));

  const has = prevHas || knockedToday;
  const firstDate = prevDate || (knockedToday ? today : "");

  // 同步回舊 localStorage 相容
  bucket.has_knock_in = has;
  bucket.knock_in_date = firstDate;

  return {
    has_knock_in: has,
    knock_in_date: firstDate,
    has_ki_breach: has,
    first_ki_breach_time: firstDate
  };
}
...
function calcFCN(fcn, market, poolMap = {}, earlyExitState = {}, knockInState = {}) {
  ...
  return {
    ...fcn,
    // 新增：把新持久欄位回傳給 cache / UI / JSON 預覽
    early_exit_record: earlyExit.early_exit_record,
    has_ki_breach: knockInInfo.has_ki_breach,
    first_ki_breach_time: knockInInfo.first_ki_breach_time,
...
export function runM2HealthEngine({ fcnPool = [], marketRuntime = {}, pool30 = [] }) {
  ...
  // 仍保留寫回 localStorage，避免你舊資料流立刻斷掉
