...
function buildIndustryImpactMap(news, stockPool, sensitivityMap, max) {
  ...
  stockPool.forEach((stock) => {
    const hitSector = news.affected_sectors?.includes(stock.sector);
    const hitSub =
      news.affected_subsectors?.length > 0
        ? news.affected_subsectors.includes(stock.subsector)
        : false;

    if (news.affected_subsectors?.length > 0) {
      if (!hitSub) return;
    } else {
      if (!hitSector) return;
    }
