...
AI v0 only: residual persistence, stability, and overlay suggestion.
No real ML model is used in this version.
"""
...
def explain(symbols: list[str], direction: str, residual: float | None) -> str:
    basket = ", ".join(symbols) if symbols else "this basket"
    if residual is None:
        return f"{basket} has market coupon history, but no comparable M8 fair rate yet. Keep collecting quotes before changing the template."
    if direction == "market_higher":
        return f"The market pays more coupon for {basket} than M8 fair value by about {round(residual, 2)} points. This may indicate higher volatility, stronger demand, or a theme that needs overlay."
    if direction == "market_lower":
        return f"The market coupon for {basket} is below M8 fair value by about {round(abs(residual), 2)} points. This may mean the model is too generous or the quote is conservative."
    return f"{basket} is close to M8 fair value. The template looks stable for now."
...
def build_memory() -> dict[str, Any]:
    ...
    for row in market_rows:
        grouped[key_of(normalize_symbols(row.get("symbols")))].append(row)
    ...
    for key in sorted(set(grouped.keys()) | set(surface_small.keys())):
        ...
        coupons = [as_number(row.get("coupon_pct")) for row in rows]
        ...
        row_dirs = [residual_direction(c - avg_new_fair) for c in coupons if c is not None and avg_new_fair is not None]
        ...
        latest = max(rows, key=lambda row: str(row.get("generated_at") or "")) if rows else {}
        ids = [str(row.get("product_id") or row.get("id") or "").strip() for row in rows]
        ...
        memories.append({
            ...
            "parent_template": surface_row.get("parent_template") or "F_OTHERS_M7_BASKET_DRIVEN",
            "small_template_key": key,
            "symbols": symbols,
            "sample_count": sample_count,
            "last_market_coupon": as_number(latest.get("coupon_pct")) if latest else market_coupon,
            ...
            "last_seen_date": latest.get("generated_at") or surface_row.get("latest_observation_date"),
            ...
            "explainability": explain(symbols, direction, residual),
    ...
    return {
        ...
        "ai_v0": {"residual_persistence": "sample_count plus repeated residual direction", "stability": "near_fair is stable; large residual or overlay flag needs review", "overlay_suggestion": "rule-based suggestion from residual size and direction", "is_real_ml": False},
        "memories": memories
    }
