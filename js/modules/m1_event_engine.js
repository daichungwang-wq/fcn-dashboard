/* ==========================================
   振宇 FCN 系統｜M1 Event Engine V1.1
   功能：
   1. 讀 news_items
   2. 依 type 產生 impact_map
   3. 彙總成 stock_event_map
   4. 直接吃 pool30.json（sector / subsector / category）
========================================== */
...
/* ------------------------------------------
   ...
   - 沒有 affected_subsectors 時：用 sector
   公式：industry_score = SID
------------------------------------------ */
function buildIndustryImpactMap(news, stockPool) {
  const result = {};
  const sid = toNumber(news.sid_score, 0);
  ...
  stockPool.forEach((stock) => {
    ...
    addImpact(result, stock.symbol, sid, news.id);
  });
...
export function buildNewsImpactMap(
  ...
  if (news.type === "industry") {
    return buildIndustryImpactMap(news, stockPool);
  }
