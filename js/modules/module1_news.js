...
function marketInterpretation(key, current, previous) {
  ...
  switch (key) {
    ...
    case "us10y":
    case "us20y":
      return delta > 0 ? "利率上升（⚠️ 壓估值）" : "利率下降（支撐股市）";
    ...
    case "cpi":
      return delta > 0 ? "通膨升溫（偏利空）" : "通膨下降（偏利多）";

    case "ppi":
      return delta > 0 ? "生產端通膨升溫" : "生產端通膨下降（偏利多）";
...
function marketCard(label, key, data) {
  ...
  return `
    <div class="market-card">
      <div class="market-card-title">${label}</div>
      <div class="market-card-value">
        ${formatValue(key, current)}
        <span class="market-card-sub">（${formatValue(key, previous)}，<span class="${deltaClass}">${deltaText}</span>）</span>
      </div>
      <div class="market-card-note">→ ${marketInterpretation(key, current, previous)}</div>
    </div>
  `;
}
...
function renderNewsItem(item, idx, prefix) {
  ...
  return `
    <div class="news-card">
      ...
      <div class="news-expand" onclick="toggleNews('${id}', this)">
        點擊展開
      </div>
...
export function renderModule1News(newsData, marketData) {
  ...
  return `
    <div class="module1">
      ...
      ${renderMarketSection(marketData)}
...
window.toggleNews = function(id, el) {
  ...
  if (el) {
    el.textContent = isHidden ? "點擊收合" : "點擊展開";
  }
};
...
window.rerunDecision = function() {
  alert("這一版先完成排版與覆核流程。下一步再正式連動 Module3 重算。");
};
