(function(){
  ...
  function activePosition(f){return String(f.status||'').trim().toLowerCase()==='active' && f.has_position!==false && f.is_portfolio!==false;}
  ...
  async function loadPool(){
    ...
    }catch(e){console.error(e);runtimeMeta.textContent='讀取 fcn_pool.json 失敗';poolRows=[];}
  }
  ...
  function templateRows(){
    ...
    if(selectedTemplate==='bank'&&ui.bank)rows=rows.filter(f=>String(f.tw_bank||f.bank||'').includes(ui.bank));
    if(selectedTemplate==='rayray')rows=rows.filter(f=>String(f.fcn_id||'').includes('睿睿'));
    if(selectedTemplate==='single'&&ui.single){const single=String(ui.single||'').trim().toUpperCase();rows=rows.filter(f=>String(f.fcn_id||'').toUpperCase().includes(single));}
    if(selectedTemplate==='custom'&&ui.custom){const ids=String(ui.custom||'').toUpperCase().split(/[\n, ]+/).map(x=>x.trim()).filter(Boolean);rows=rows.filter(f=>ids.some(id=>String(f.fcn_id||'').toUpperCase().includes(id)));}
    return rows;
  }

  function buildProjection(){
    const start=new Date(), end=new Date();end.setMonth(end.getMonth()+cfg.months);const rows=[];
    templateRows().forEach(f=>{const e=new Date(entryDate(f));if(Number.isNaN(e.getTime()))return;const mat=new Date(maturityDate(f));const stop=Number.isNaN(mat.getTime())?end:mat;let coupon=addDays(e,cfg.firstOffset),seq=1;while(coupon<=end&&coupon<=stop&&seq<=48){const cash=addDays(coupon,cfg.cashBuffer);if(cash>=start){const interest=n(f.amt,0)*n(f.rate,0)/100/12;rows.push({month:ym(cash),cash_in_date:ymd(cash),coupon_date:ymd(coupon),seq,fcn_id:f.fcn_id||'',status:f.status||'',bank:f.tw_bank||f.bank||'',basket:getBasket(f).join('/'),currency:f.currency||'USD',amt:n(f.amt,0),rate:n(f.rate,0),interest});}coupon=addDays(coupon,cfg.interval);seq++;}});
  ...
  function renderMonthlyCards(mon){
    const max=Math.max(1,...mon.map(r=>n(r.interest,0)));
    return `<div class="ip-month-cards">${mon.map(r=>{const pct=Math.max(3,Math.min(100,n(r.interest,0)/max*100));return `<div class="ip-month-card"><div class="ip-month-head"><b>${r.month}</b><span>${r.currency}</span></div><div class="ip-month-money">${r.currency} ${fmt(r.interest,2)}</div><div class="ip-month-sub">${r.count} 筆入帳</div><div class="ip-month-bar"><span style="width:${pct}%"></span></div></div>`}).join('')}</div>`;
  }

  function renderTables(){
    const mon=monthlySummary();
    if(!projectionRows.length)return `<div class="ip-empty">目前模板沒有可推估資料。只計算 status = active 且 has_position / is_portfolio 未被標 false 的 FCN；closed 不計息。</div>`;
    return `<h3>7C. 月別總表</h3>${renderMonthlyCards(mon)}<details class="ip-month-detail"><summary>月別表格明細</summary><div class="ip-table-wrap"><table class="ip-table"><thead><tr><th>Month</th><th>Currency</th><th>Count</th><th>Estimated Interest</th></tr></thead><tbody>${mon.map(r=>`<tr><td><b>${r.month}</b></td><td>${r.currency}</td><td>${r.count}</td><td>${r.currency} ${fmt(r.interest,2)}</td></tr>`).join('')}</tbody></table></div></details><h3>7D. FCN 明細表</h3><div class="ip-table-wrap"><table class="ip-table"><thead><tr><th>Cash-in</th><th>Coupon Date</th><th>#</th><th>FCN</th><th>Status</th><th>Bank</th><th>Basket</th><th>Amt</th><th>Rate</th><th>Interest</th></tr></thead><tbody>${projectionRows.map(r=>`<tr><td>${r.cash_in_date}</td><td>${r.coupon_date}</td><td>${r.seq}</td><td><b>${esc(r.fcn_id)}</b></td><td>${esc(r.status)}</td><td>${esc(r.bank)}</td><td>${esc(r.basket)}</td><td>${r.currency} ${fmt(r.amt,0)}</td><td>${fmt(r.rate,2)}%</td><td>${r.currency} ${fmt(r.interest,2)}</td></tr>`).join('')}</tbody></table></div>`;
  }

  function render(){
    buildProjection();const bankDisabled=selectedTemplate!=='bank'?'disabled':'',singleDisabled=selectedTemplate!=='single'?'disabled':'',customDisabled=selectedTemplate!=='custom'?'disabled':'';
    ...
    injectMonthlyCardCss();bind();
  }

  function injectMonthlyCardCss(){
    if(document.getElementById('ipMonthlyCardCss'))return;
    const st=document.createElement('style');st.id='ipMonthlyCardCss';st.textContent=`.ip-month-cards{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:10px 0 12px}.ip-month-card{border:1px solid #e5e7eb;border-radius:16px;background:linear-gradient(135deg,#fff,#f8fafc);padding:12px;box-shadow:0 2px 8px rgba(15,23,42,.04)}.ip-month-head{display:flex;justify-content:space-between;gap:8px;align-items:center;color:#334155}.ip-month-head b{font-size:15px}.ip-month-head span{font-size:11px;font-weight:900;color:#64748b;background:#eef2ff;border-radius:999px;padding:3px 7px}.ip-month-money{font-size:20px;font-weight:950;margin-top:8px}.ip-month-sub{font-size:12px;color:#64748b;margin-top:4px}.ip-month-bar{height:7px;background:#e2e8f0;border-radius:999px;margin-top:10px;overflow:hidden}.ip-month-bar span{display:block;height:100%;background:#111;border-radius:999px}.ip-month-detail{border:1px solid #e5e7eb;border-radius:14px;background:#fff;padding:10px;margin:8px 0 14px}.ip-month-detail summary{cursor:pointer;font-weight:950;color:#334155}@media(max-width:1100px){.ip-month-cards{grid-template-columns:repeat(2,1fr)}}@media(max-width:720px){.ip-month-cards{grid-template-columns:1fr}}`;document.head.appendChild(st);
  }
  ...
  function bind(){document.querySelectorAll('[data-ip-template]').forEach(b=>b.addEventListener('click',()=>setTemplate(b.dataset.ipTemplate)));['ipBank','ipSingle','ipCustom','ipMonths','ipFirst','ipInterval','ipBuffer'].forEach(id=>{const el=document.getElementById(id);if(!el)return;el.addEventListener('input',()=>{syncUiFromDom();setTimeout(render,0)});el.addEventListener('change',()=>{syncUiFromDom();setTimeout(render,0)});});document.getElementById('ipRun')?.addEventListener('click',()=>{syncUiFromDom();render()});document.getElementById('ipCopyJson')?.addEventListener('click',()=>{navigator.clipboard?.writeText(JSON.stringify({template:selectedTemplate,filters:ui,config:cfg,rule:'status must equal active; closed not counted',rows:projectionRows},null,2));});document.getElementById('ipCopyCsv')?.addEventListener('click',()=>{navigator.clipboard?.writeText(toCsv());});document.getElementById('reloadBtn')?.addEventListener('click',()=>location.reload());}
