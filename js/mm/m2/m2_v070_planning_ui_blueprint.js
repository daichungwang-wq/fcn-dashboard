// ============================================================
// MM/M2 v070c Planning UI
// Purpose: Section 3 Strategy Gap First, using robust visible allocation parser.
// ============================================================
(function(){
  ...
  function parseAllocationFromText(){
    const out={}; Object.keys(TARGETS).forEach(k=>out[k]=null);
    ...
    Object.keys(TARGETS).forEach(k=>{
      const idx=text.indexOf(k); if(idx<0) return;
      const slice=text.slice(idx, idx+90);
      const m=slice.match(/USD\s*([0-9,]+(?:\.\d+)?)/i);
      if(m){out[k]=Math.floor(Number(m[1].replace(/,/g,''))/10000);}
    });
    return out;
  }

  function buildStaticGapRows(){
    const current=parseAllocationFromText();
  ...
  function stageData(){
    ...
    const strategy=first?.strategy||'待接資料'; const need=first?.need_wan;
  ...
  function renderStrategyGap(){const rows=buildStaticGapRows(); const body=rows.map(r=>`<tr class="${r.gap_pct!=null&&r.gap_pct<0?'m2v070-under':'m2v070-over'}"><td><b>${r.strategy}</b></td><td>${r.current_wan==null?'-':wan(r.current_wan)}</td><td>${r.real_pct==null?'-':fmt(r.real_pct,1)+'%'}</td><td>${fmt(r.target_pct,0)}%</td><td>${r.gap_pct==null?'-':fmt(r.gap_pct,1)+'%'}</td><td>${r.need_wan==null?'-':wan(r.need_wan)}</td><td>${r.gap_pct==null?'待接資料':r.gap_pct<0?'Underweight':'OK / Over'}</td></tr>`).join('');return `<div class="m2v070-panel" id="planner-strategy-refill"><h3>C. 投資策略補單｜Strategy Gap First</h3><div class="m2v070-note"><b>口徑：</b>四類策略全部用 Total Plan Base 140萬計算 Real / Target / Gap；Gap% 最負者優先補。</div><div class="table-wrap" style="margin-top:10px"><table class="m2v070-table"><thead><tr><th>策略類別</th><th>目前金額</th><th>Real</th><th>Target</th><th>Gap</th><th>待補萬</th><th>狀態</th></tr></thead><tbody>${body}</tbody></table></div><div class="m2v070-note">v070c：從既有「Planning Base 配置 / Real vs Target」區塊逐類讀取 USD 金額，再用 140萬重算。</div></div>`;}
  ...
  function renderOutput(){const plan={version:'v070c_visible_allocation',unit:'wan_usd',strategy_gap_rows:buildStaticGapRows(),base_policy:{total_plan_base_wan:140,fubon_plan_base_wan:90,sinopac_plan_base_wan:50},stage_capital:[{stage_id:'priority',available_wan:5},{stage_id:'short_term',available_wan:15},{stage_id:'strategic',available_wan:15}],step_display_rule:'Step N｜策略類別｜銀行｜金額'};window.__M2_ALLOCATION_PLAN_BLUEPRINT__=plan;return `<div class="m2v070-panel" id="planner-output"><h3>E0. Planner Output｜給第 4 區讀</h3><div class="m2v070-note">目前輸出：<b>window.__M2_ALLOCATION_PLAN_BLUEPRINT__</b>。</div><details class="m2v070-trace"><summary>查看 blueprint JSON</summary><pre style="white-space:pre-wrap;font-size:12px;background:#0f172a;color:#e5e7eb;border-radius:12px;padding:10px;max-height:320px;overflow:auto">${JSON.stringify(plan,null,2)}</pre></details></div>`;}
  function buildHtml(){return `<div class="m2v070-wrap" id="m2v070PlanningBlueprint"><div class="m2v070-head"><b>M2 v070c Planning UI</b><br>第 3 區接既有配置資料，策略缺口用 140萬靜態母數；可補金額用 5萬 / 15萬 / 15萬 分階段控制。</div>${renderKpis()}${renderStrategyGap()}${renderStageCards()}${renderOutput()}</div>`;}
  ...
  new MutationObserver(()=>setTimeout(inject,180)).observe(document.documentElement,{childList:true,subtree:true,characterData:true});
