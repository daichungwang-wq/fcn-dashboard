...
// ---------- Summary ----------
export function buildFcnSummary(pool, usd=33){
...
// ---------- Table ----------
export function buildFcnTable(pool, usd=33){
  return pool
...
// ---------- 12個月時間軸 ----------
export function buildCashflow12M(pool, usd=33){
  const active = pool.filter(x=>x.status==="active");
  const arr = [];
