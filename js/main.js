...
function injectUtilityStyles() {
  ...
  style.textContent = `
    ...
    .stock-card,.position-card,.news-card,.model-card {
      background:#fff;
      border:1px solid #e5e7eb;
      border-radius:18px;
      box-shadow:0 2px 10px rgba(0,0,0,.05);
  ...
  container.innerHTML = `
    ...
    ${newsGroups.map((group) => `
      <div class="model-card">
        ...
        <div id="news-group-${group.key}" style="margin-top:12px;">
          ${group.items.map((item) => `
            <div class="news-card">
              <div class="stock-head" style="font-size:18px;">${item.title}</div>
              <div class="line">${item.summary}</div>
              <div class="line">
                <strong>影響：</strong>${item.impact}
               　<strong>強度：</strong>${item.strength}
               　<strong>方向：</strong>
                <span class="${item.direction.includes("正") ? "delta-positive" : item.direction.includes("負") ? "delta-negative" : ""}">
                  ${item.direction}
                </span>
              </div>
            </div>
...
function renderM3Model() {
  ...
  el.innerHTML = `
    <div class="model-card">
      <div class="btn-row">
        <button class="btn" onclick="toggleM3Model()">展開完整模型說明</button>
      </div>

      <div id="m3-model-detail">
...
function renderM3Summary() {
  ...
  el.innerHTML = `
    <div class="summary-grid">
      ...
      <div class="summary-card">
        <div class="summary-title">Event 上修</div>
        <div class="summary-value up">${up}</div>
      </div>
      <div class="summary-card">
        <div class="summary-title">Event 下修</div>
        <div class="summary-value down">${down}</div>
      </div>
    </div>
...
window.toggleNewsGroup = function (key) {
  const el = document.getElementById(`news-group-${key}`);
  if (!el) return;
...
window.toggleM3Model = function () {
  const el = document.getElementById("m3-model-detail");
  if (!el) return;
  el.style.display = el.style.display === "none" ? "block" : "none";
};
