...
#marketWorkspaceContent .v69-cards{display:flex!important;flex-wrap:nowrap!important;gap:12px!important;overflow-x:auto!important;overflow-y:hidden!important;padding:4px 2px 10px!important;width:100%!important;max-width:100%!important}
#marketWorkspaceContent .v69-card{display:block!important;flex:0 0 322px!important;width:322px!important;max-width:322px!important;min-width:322px!important;border:1px solid #e5e7eb!important;border-radius:18px!important;background:#fff!important;padding:12px!important;box-shadow:0 2px 8px rgba(15,23,42,.05)!important;overflow:hidden!important;white-space:normal!important}
...
#marketWorkspaceContent .v69-card-top{display:flex!important;justify-content:space-between!important;gap:8px!important;align-items:flex-start!important}
#marketWorkspaceContent .v69-card-title{display:flex!important;gap:6px!important;align-items:center!important;font-weight:950!important;font-size:13px!important;line-height:1.35!important;min-width:0!important;white-space:normal!important}
#marketWorkspaceContent .v69-card-title input{flex:0 0 auto!important;margin:0!important}
#marketWorkspaceContent .v69-amt{display:flex!important;align-items:center!important;gap:4px!important;font-size:12px!important;color:#334155!important;white-space:nowrap!important}
#marketWorkspaceContent .v69-amt input{width:58px!important;min-width:58px!important;border:1px solid #cbd5e1!important;border-radius:8px!important;padding:5px!important;text-align:right!important;font-weight:900!important;background:#fff!important;color:#111!important}
#marketWorkspaceContent .v69-source{font-size:12px!important;color:#64748b!important;margin-top:5px!important}
#marketWorkspaceContent .v69-chips{display:flex!important;gap:5px!important;flex-wrap:wrap!important;margin:8px 0!important}
...
#marketWorkspaceContent .v69-terms b{font-size:17px!important}
#marketWorkspaceContent .v69-fair{border-top:1px dashed #e5e7eb!important;padding-top:7px!important}
#marketWorkspaceContent .v69-why{background:#f8fafc!important;border:1px solid #e5e7eb!important;border-radius:12px!important;padding:8px!important}
...
@media(max-width:900px){#marketWorkspaceContent .v69-bp-grid{grid-template-columns:1fr!important}#marketWorkspaceContent .v69-slot-head{display:block!important}#marketWorkspaceContent .v69-slot-status{display:inline-block!important;margin-top:8px!important}#marketWorkspaceContent .v69-card{flex-basis:288px!important;width:288px!important;max-width:288px!important;min-width:288px!important}}
</style>`}
  ...
  function candidateCard(need,c){
    ...
    return `<div class="v69-card v69-grade-${grade}" data-v69-need="${need.need_id}" data-v69-candidate="${c.product_id}">
      <div class="v69-card-top">
        ...
        <div class="v69-amt">建議 <input class="v69-amt-input" data-v69-need="${need.need_id}" data-v69-candidate="${c.product_id}" type="number" min="0" step="1" value="${amount}"> 萬</div>
      </div>
      <div class="v69-source">${c.source}｜${c.generated_at||'-'}｜上手 ${c.upstream_bank||'-'}｜Score ${fmt(c.candidate_score,2)}</div>
      <div class="v69-chips">${(c.symbols||[]).map(s=>`<span class="v69-chip">${s}</span>`).join('')}</div>
      <div class="v69-terms"><b>${fmt(c.coupon_pct,2)}%</b>｜${fmt(c.tenor_month,0)}M｜${c.barrier_type||'NA'} ${c.memory_type||''}｜Strike/KI ${strikeText}/${kiText}</div>
      <div class="v69-fair"><b>Market ${fmt(c.coupon_pct,2)}%</b>｜M8 Fair ${fmt(c.m8_fair_rate,2)}%｜Gap ${pct(c.fair_gap)}｜Final Ref ${fmt(c.final_fair_ref,2)}%</div>
      ...
      <div class="v69-why">配對 ${fmt(c.planner_need_match,1)}/10｜Risk ${fmt(c.risk_fit_score,1)}/10｜Bank ${fmt(c.bank_fit_score,1)}/10${penalties}</div>
    </div>`;
  }
