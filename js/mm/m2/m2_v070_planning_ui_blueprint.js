...
// v070b: Connect best-effort data from existing visible Planning Base table.
// ============================================================
(function(){
  ...
  function injectCss(){
    ...
    style.textContent = `
      ...
      .m2v070-under{background:#fff7ed}.m2v070-over{background:#f8fafc}
      ...
      @media(max-width:1000px){.m2v070-kpis,.m2v070-result{grid-template-columns:1fr}}
    `;
    document.head.appendChild(style);
  }

  function parseWanText(s){
    s=String(s||'').replace(/,/g,'');
    const m=s.match(/USD\s*([0-9.]+)/i) || s.match(/([0-9.]+)\s*萬/);
    if(!m) return null;
    const val=Number(m[1]);
    if(!Number.isFinite(val)) return null;
    return /萬/.test(s)?val:Math.floor(val/10000);
  }

  function scrapeAllocRows(){
    const out={};
    Object.keys(TARGETS).forEach(k=>{out[k]=null;});
    const root=document.getElementById('planner-allocation') || document.getElementById('bottomQuery') || document.body;
    const text=root.innerText||'';
    Object.keys(TARGETS).forEach(k=>{
      const idx=text.indexOf(k);
      if(idx<0) return;
      const slice=text.slice(idx, idx+220);
      const usd=slice.match(/USD\s*([0-9,]+)/i);
      if(usd) out[k]=Math.floor(Number(usd[1].replace(/,/g,''))/10000);
    });
    return out;
  }

  function buildStaticGapRows(){
    const current=scrapeAllocRows();
    return Object.keys(TARGETS).map(k=>{
      ...
      if(a.gap_pct==null && b.gap_pct==null) return 0;
      if(a.gap_pct==null) return 1;
      if(b.gap_pct==null) return -1;
      return a.gap_pct-b.gap_pct;
    });
  }

  function firstUnder(rows){return rows.filter(r=>r.gap_pct!=null&&r.gap_pct<0).sort((a,b)=>a.gap_pct-b.gap_pct)[0];}

  function stageData(){
    const rows=buildStaticGapRows();
    const first=firstUnder(rows);
    const strategy=first?.strategy||'待接資料';
    const need=first?.need_wan;
    const s1Amt=Math.min(5, need==null?5:need);
    return [
      {id:'priority',title:'第一階段｜優先規劃',available:5,planned:s1Amt,remaining:5-s1Amt,status:'結果優先',resultStrategy:`${strategy} ${wan(s1Amt)}｜gap% 最負者先補`,resultBank:s1Amt>3?'永豐 3萬、富邦 '+wan(s1Amt-3):'永豐 / 富邦依 min lot 判斷',note:'確定 release，可立即規劃。每一步補完後重算策略 gap 與銀行 gap。',steps:[[strategy,'永豐',Math.min(3,s1Amt),`Step 1｜${strategy}｜永豐｜${wan(Math.min(3,s1Amt))}：策略類別放最前面；先補 gap% 最負者。`],[strategy,'富邦',Math.max(0,s1Amt-3),`Step 2｜${strategy}｜富邦｜${wan(Math.max(0,s1Amt-3))}：若本階段尾數低於永豐 min 3萬，改富邦補尾數。`]]},
      {id:'short_term',title:'第二階段｜短期規劃',available:15,planned:null,remaining:null,status:'條件式',resultStrategy:'承接第一階段剩餘缺口，繼續以 gap% 最負者優先。',resultBank:'依銀行 gap% 與 min/max lot 決定。',note:'候選 release 成立後再啟用；若四類缺口已補滿，多餘資金保留，不硬補。',steps:[['待推演','待分配',0,'第二階段將讀取第一階段後的 remaining need，再重新排序。']]},
      {id:'strategic',title:'第三階段｜策略佈局',available:15,planned:null,remaining:null,status:'候補',resultStrategy:'只在市場單條件合理時啟用。',resultBank:'需同時通過銀行水位、M8 gap、風險限制。',note:'未來一月候補資金，不視為本月必做。',steps:[['候補','待分配',0,'第三階段預設只保留；市場條件好才轉交第 4 區配對。']]}
    ];
  }
  ...
  function renderStrategyGap(){
    ...
    return `<div class="m2v070-panel" id="planner-strategy-refill"><h3>C. 投資策略補單｜Strategy Gap First</h3><div class="m2v070-note"><b>口徑：</b>四類策略全部用 Total Plan Base 140萬計算 Real / Target / Gap；Gap% 最負者優先補。補單模式只做風控限制，不主導排序。</div><div class="table-wrap" style="margin-top:10px"><table class="m2v070-table"><thead><tr><th>策略類別</th><th>目前金額</th><th>Real</th><th>Target</th><th>Gap</th><th>待補萬</th><th>狀態</th></tr></thead><tbody>${body}</tbody></table></div><div class="m2v070-note">資料來源：先從目前第 3 區既有 Planning Base / 配置重建畫面抓取可見金額；分母固定 140萬。若看到 -，代表原畫面尚未渲染該策略金額，重新進第 3 區或按執行後會補上。</div></div>`;
  }

  function renderStageCards(){return `<div class="m2v070-panel" id="planner-stage-simulation"><h3>D. 補單步驟推演｜Stage Result First</h3><div class="m2v070-note">預設顯示各階段結果；中間推演過程隱藏。每一步格式固定為：<b>Step N｜策略類別｜銀行｜金額</b>。</div><div class="m2v070-actions"><button type="button" data-m2v070="expand">全部展開</button><button type="button" data-m2v070="collapse">全部收合</button></div>${stageData().map(s=>`<div class="m2v070-stage"><div class="m2v070-stage-head"><div><div class="m2v070-stage-title">${s.title}</div><div class="m2v070-stage-sub">可用 ${wan(s.available)}｜已規劃 ${s.planned==null?'待推演':wan(s.planned)}｜剩餘 ${s.remaining==null?'待推演':wan(s.remaining)}</div></div><span class="m2v070-pill">${s.status}</span></div><div class="m2v070-result"><div><label>策略結果</label>${s.resultStrategy}</div><div><label>銀行配置</label>${s.resultBank}</div><div><label>階段說明</label>${s.note}</div></div><details class="m2v070-trace"><summary>展開推演過程</summary>${s.steps.filter(x=>n(x[2],0)>0||x[0]==='待推演'||x[0]==='候補').map((x,i)=>`<div class="m2v070-step"><b>Step ${i+1}｜${x[0]}｜${x[1]}｜${wan(x[2])}</b><br>${x[3]}</div>`).join('')}<div class="m2v070-note">分配規則：allocate one executable lot → recompute strategy gap / bank gap → next priority → 若低於銀行 min 或類型已滿足，停止或切換銀行。</div></details></div>`).join('')}</div>`;}

  function renderOutput(){const rows=buildStaticGapRows();const plan={version:'v070b_visible_data',unit:'wan_usd',base_policy:{total_plan_base_wan:140,fubon_plan_base_wan:90,sinopac_plan_base_wan:50,gap_base_rule:'strategy_gap_uses_static_total_plan_base_140',release_rule:'release_controls_stage_capital_only'},strategy_gap_rows:rows,stage_capital:[{stage_id:'priority',available_wan:5},{stage_id:'short_term',available_wan:15},{stage_id:'strategic',available_wan:15}],step_display_rule:'Step N｜策略類別｜銀行｜金額'};window.__M2_ALLOCATION_PLAN_BLUEPRINT__=plan;return `<div class="m2v070-panel" id="planner-output"><h3>E0. Planner Output｜給第 4 區讀</h3><div class="m2v070-note">目前輸出：<b>window.__M2_ALLOCATION_PLAN_BLUEPRINT__</b>。下一階段把第 4 區 Market FCN Match 改讀正式 allocation_steps。</div><details class="m2v070-trace"><summary>查看 blueprint JSON</summary><pre style="white-space:pre-wrap;font-size:12px;background:#0f172a;color:#e5e7eb;border-radius:12px;padding:10px;max-height:320px;overflow:auto">${JSON.stringify(plan,null,2)}</pre></details></div>`;}

  function buildHtml(){return `<div class="m2v070-wrap" id="m2v070PlanningBlueprint"><div class="m2v070-head"><b>M2 v070b Planning UI</b><br>第 3 區開始接可見資料：策略缺口用 140萬靜態母數；可補金額用 5萬 / 15萬 / 15萬 分階段控制。</div>${renderKpis()}${renderStrategyGap()}${renderStageCards()}${renderOutput()}</div>`;}
  ...
  document.addEventListener('click',function(ev){const btn=ev.target.closest('[data-planner-nav]');if(btn){const map={strategy:'planner-strategy-refill',stage:'planner-stage-simulation',output:'planner-output'};setTimeout(()=>{inject();const el=document.getElementById(map[btn.dataset.plannerNav]);if(el) el.scrollIntoView({behavior:'smooth',block:'start'});},420);}else{setTimeout(inject,220);}},true);
  new MutationObserver(()=>setTimeout(inject,140)).observe(document.documentElement,{childList:true,subtree:true,characterData:true});
