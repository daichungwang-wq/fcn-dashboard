(function(){
  ...
  const statusText=x=>String(x&& (x.status||x.zone||x.state||x.label)||'').toLowerCase();
  const amountWan=x=>{
    const v=n(x&& (x.amount_wan??x.amount??x.principal_wan??x.principal??x.exposure??x.total_exposure),0);
    return v>10000?v/10000:v;
  };
  function getBank(row){
    const raw=String(row&& (row.bank||row.broker||row.source||row.bank_source)||'').toLowerCase();
  ...
  function strategyAmountsFromRows(rows){
    const out={};
    arr(rows).forEach(r=>{const k=r.strategy||r.strategy_title||r.category||r.operation_type; if(k) out[k]=(out[k]||0)+amountWan(r);});
    return out;
  }
  function buildPlanner(inputBank){
    const steps=[
    ...
    const stageSummary={}; const strategySummary={}; const bankSummary={};
  ...
  function build(state){
    ...
    let bankInput=inputByBank(rows);
    if(!bankInput['富邦']&&!bankInput['永豐']){
      const total=sum(rows,amountWan);
      if(total>0) bankInput={'富邦':Math.round(total*0.75),'永豐':Math.round(total*0.25)};
    }
    ...
    const danger=countAmt(rows,x=>/danger|ki|risk|破|下限|危險/.test(statusText(x))||n(x&&x.total_exposure,0)>=15);
    const tracking=countAmt(rows,x=>/watch|tracking|追蹤|觀察/.test(statusText(x))||(n(x&&x.total_exposure,0)>=8&&n(x&&x.total_exposure,0)<15));
    const health=countAmt(rows,x=>/healthy|safe|健康|安全/.test(statusText(x))||(n(x&&x.total_exposure,0)>0&&n(x&&x.total_exposure,0)<8));
    const planner=buildPlanner(bankInput);
    const selected=window.__M2_MARKET_FCN_SELECTION_SUMMARY__||null;
    return {
      version:'mm_m2_cashflow_engine_v1_full',
