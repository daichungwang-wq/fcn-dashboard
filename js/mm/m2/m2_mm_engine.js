...
const MODULE_META={summary:['1. Summary Dashboard','資金水位 / 配置 / 健康 / 預計出場'],zones:['2. Holding Zones','到期專區 / 預計提前到期 / Danger / Watch / Healthy'],planning:['3. Maturity Cashflow','Planner A/B/C/D/E0：到期資金流與佈局重建'],market:['4. Market FCN Analysis','A. 單筆 M8 定價 / B. 整批模板決策區'],management:['5. FCN / Stock Management','完整 FCN 查詢 + 股票風險分析'],pool:['6. Pool Manual Ops','手動建新單 / 編輯 / 複製 / Soft Delete / 匯出']};
...
async function runSingleCheck(){
  ...
  try{
    ...
    box.innerHTML=renderSingleResult(res,input);
  }catch(err){
    console.error(err);
...
runBtn.addEventListener('click',loadData);reloadBtn.addEventListener('click',loadData);bindMenu();bindMaturitySubnav();loadData();
