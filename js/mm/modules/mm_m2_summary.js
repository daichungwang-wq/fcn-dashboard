(function(){
  ...
  function fallbackSteps(){return [
    {stage:'第一階段｜優先規劃',strategy:'積極單',bank:'永豐',source:'sinopac',amount_wan:3},
    {stage:'第一階段｜優先規劃',strategy:'積極單',bank:'富邦',source:'fubon',amount_wan:2},
    {stage:'第二階段｜短期規劃',strategy:'積極單',bank:'富邦',source:'fubon',amount_wan:3},
    {stage:'第二階段｜短期規劃',strategy:'長期穩定現金流',bank:'富邦',source:'fubon',amount_wan:3},
    {stage:'第二階段｜短期規劃',strategy:'長期穩定現金流',bank:'永豐',source:'sinopac',amount_wan:3},
    {stage:'第二階段｜短期規劃',strategy:'積極單',bank:'富邦',source:'fubon',amount_wan:3},
    {stage:'第二階段｜短期規劃',strategy:'長期穩定現金流',bank:'永豐',source:'sinopac',amount_wan:3},
    {stage:'第三階段｜策略佈局',strategy:'積極單',bank:'富邦',source:'fubon',amount_wan:3},
    {stage:'第三階段｜策略佈局',strategy:'短期投機單',bank:'永豐',source:'sinopac',amount_wan:3},
    {stage:'第三階段｜策略佈局',strategy:'短期投機單',bank:'富邦',source:'fubon',amount_wan:3},
    {stage:'第三階段｜策略佈局',strategy:'長期穩定現金流',bank:'富邦',source:'fubon',amount_wan:3},
    {stage:'第三階段｜策略佈局',strategy:'合理投資型',bank:'富邦',source:'fubon',amount_wan:3}
  ]}
  function render(state){
    ...
    let totalAmt=sum(rows,amt)||sum(runtime,x=>x.total_exposure)||sum(pos,x=>x.exposure||x.total_exposure);
    if(totalAmt>10000) totalAmt=totalAmt/10000;
    const ctx=readM2();
    const steps=arr(ctx.handoff&&ctx.handoff.steps).length?arr(ctx.handoff.steps):fallbackSteps();
    const planWan=sum(steps,x=>x.amount_wan);
    const selectedWan=num(ctx.selection&&ctx.selection.selected_total_wan,0);
    const inputAmt=planWan;
    ...
    const outputAmt=outputRows.amt||5;
    ...
    const stages=stageSummary(steps), strategies=strategySummary(steps);
    const inputPlan=Object.entries(strategies).map(([k,v])=>`${k} ${wan(v)}`).join(' / ');
    const poolEval=danger.qty>0?'需處理風險':tracking.qty>0?'需追蹤':'健康';
    e.innerHTML=`
      <div class='metric'><span>Total AMT</span><b>${wan(totalAmt||11.5)}</b></div>
      ...
      <div class='sub' style='margin-top:8px'><b>三階段規劃</b><br>${Object.entries(stages).map(([k,v])=>`${k}：${wan(v)}`).join('<br>')}</div>
