...
function getQuotePrice(symbol, poolMap) {
  return safeNum(poolMap?.[symbol]?.price);
}
...
function getEntryPrice(symbol, position) {
  ...
  // 優先順序：觸發價(若 refPrice 已存在即上面處理) → 下限價 → 執行價
  if (kiPrice !== null && kiRatio !== null && kiRatio > 0) {
    return kiPrice / kiRatio;
  }

  if (strikePrice !== null && strikeRatio !== null && strikeRatio > 0) {
...
function getLowerPrice(symbol, position) {
  ...
  // 規則：無下限價格時，用執行價格當下限價格
  return safeNum(strikeMap[symbol]);
}
...
function getDataIssues(position, rows) {
  ...
  rows.forEach(r => {
    if (r.price === null) issues.push(`${r.symbol} 無現價`);
