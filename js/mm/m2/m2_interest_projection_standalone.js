(function(){
  ...
  async function loadPool(){
    try{
      ...
      runtimeMeta.textContent=`已載入 ${poolRows.length} 筆 FCN 資料｜Standalone Interest Projection`;
    }catch(e){
      console.error(e);
  ...
  function renderKpis(){
    ...
    return `<div class="ip-grid"><div class="ip-kpi"><label>FCN Count</label><b>${fcns.length}</b></div><div class="ip-kpi"><label>Principal</label><b>USD ${fmt(totalAmt,0)}</b></div><div class="ip-kpi"><label>Projection Rows</label><b>${projectionRows.length}</b></div><div class="ip-kpi"><label>Total Interest</label><b>USD ${fmt(totalInterest,2)}</b></div></div>`;
  }
  ...
  function renderTables(){
    ...
    if(!projectionRows.length){
      return `<div class="ip-empty">目前模板沒有可推估資料。請確認 fcn_pool 是否有 active / has_position=true / entry_time。</div>`;
    }
  ...
  function toCsv(){
    const head=['month','cash_in_date','coupon_date','seq','fcn_id','bank','basket','currency','amt','rate','interest'];
