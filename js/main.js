...
// ===== 初始化 =====
async function initV73() {
  const stockRes = await fetch("data/pool.json");
  STOCKS = await stockRes.json();

  const tplRes = await fetch("data/fcn_templates.json");
...
// ===== 核心入口 =====
function runFCNEngine() {
  const allBaskets = generateAllBaskets(STOCKS, TEMPLATES);

  const scored = allBaskets.map(b => ({
    ...b,
    fcnPure: calcFCNPure(b),
    fcnEvent: calcFCNEvent(b)
  }));

  const filtered = scored.filter(b => b.fcnPure !== null);

  const ranked = filtered.map(b => ({
    ...b,
    finalScore: 0.7 * b.fcnPure + 0.3 * b.fcnEvent
  }))
  .sort((a,b) => b.finalScore - a.finalScore);

  const deduped = deduplicateBaskets(ranked);

  const finalPick = pickTopByCategory(deduped);

  renderFCN(finalPick);
}

// ===== 多生成 Basket =====
function generateAllBaskets(stocks, templates) {
  let result = [];

  templates.forEach(tpl => {
    const candidates = stocks.filter(s => s.allow_fcn !== false);

    const combos = k_combinations(candidates, tpl.basket_size);
...
// ===== FCN Pure =====
function calcFCNPure(basket) {
  const tpl = basket.template;

  if (!tpl.coupon || !tpl.tenor) return null;
  if (tpl.coupon < 10) return null;
  if (tpl.tenor > 12) return null;

  const pureAvg = avg(basket.stocks.map(s => s.preference_score || 50));

  const rateScore = scoreRate(tpl.coupon);
  const tenorScore = scoreTenor(tpl.tenor);
  const riskScore = scorePRisk(tpl.strike, tpl.ki);
  const sri = calcSRI(basket);
...
// ===== FCN Event =====
function calcFCNEvent(basket) {
  const pureAvg = avg(basket.stocks.map(s => s.preference_score || 50));
  const event = avg(basket.stocks.map(s => s.event_bias || 0));
  const vol = avg(basket.stocks.map(s => s.vol_1m || 0));

  return (
    0.5 * pureAvg +
    0.25 * event +
    0.25 * vol +
    (basket.template.eki ? 2 : 0)
  );
}
...
// ===== 下檔風險 =====
function scorePRisk(strike, ki) {
  if (!strike || !ki) return 0;
  const diff = strike - ki;
...
// ===== SRI =====
function calcSRI(basket) {
  const penalties = basket.stocks.map(s => {
    switch (s.category) {
      case "core": return 3;
...
// ===== 去重 =====
function deduplicateBaskets(baskets) {
  const result = [];

  baskets.forEach(b => {
    const exists = result.find(r => overlap(r.stocks, b.stocks) > 0.75);
    if (!exists) result.push(b);
  });
...
// ===== 分類挑選 =====
function pickTopByCategory(baskets) {
  const groups = {
    normal: [],
    aggressive: [],
    dream: [],
  ...
  baskets.forEach(b => {
    groups[b.template.risk_tag]?.push(b);
  });

  return [
    ...groups.normal.slice(0,3),
    ...groups.aggressive.slice(0,2),
    ...groups.dream.slice(0,1),
    ...groups.take_stock.slice(0,1)
  ];
}
...
function overlap(a,b){
  const setA = new Set(a.map(x=>x.symbol));
  const inter = b.filter(x=>setA.has(x.symbol));
...
function k_combinations(set, k) {
  ...
  for (let i = 0; i < set.length - k + 1; i++) {
    ...
    tailcombs.forEach(t => combs.push(head.concat(t)));
  }
  return combs;
}
...
function renderFCN(list) {
  const el = document.querySelector("#fcn-result");
  if (!el) return;

  el.innerHTML = list.map(b => `
    <div style="margin-bottom:20px;">
      <b>${b.template.name}</b><br/>
      ${b.stocks.map(s=>s.symbol).join(" / ")}<br/>
      Pure: ${b.fcnPure.toFixed(1)} |
      Event: ${b.fcnEvent.toFixed(1)} |
      Final: ${b.finalScore.toFixed(1)}
    </div>
  `).join("");
}
